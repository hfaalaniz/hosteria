...
async function getTenantId(c) {
  ...
  if (!slug) return null;
  const t = await c.env.DB.prepare('SELECT id FROM tenants WHERE slug=? AND activo=1').bind(slug).first();
  return t?.id ?? null;
}
...
reservasWeb.get('/config', async c => {
  ...
  const { results } = await c.env.DB.prepare(`SELECT clave,valor FROM configuracion_mt WHERE tenant_id=? AND clave IN (${claves.map(()=>'?').join(',')})`).bind(tenant_id, ...claves).all();
...
reservasWeb.get('/habitaciones', async c => {
  ...
  const { results } = await c.env.DB.prepare(`
    ...
    FROM habitaciones h JOIN tipos_habitacion t ON h.tipo_id=t.id WHERE h.tenant_id=? AND t.activo=1 ORDER BY h.piso,h.numero`).bind(hoy, hoy, tenant_id).all();
...
reservasWeb.get('/disponibilidad', async c => {
  ...
  const { results } = await c.env.DB.prepare(`
    ...
    FROM habitaciones h JOIN tipos_habitacion t ON h.tipo_id=t.id WHERE h.tenant_id=? AND t.activo=1 AND h.estado NOT IN ('mantenimiento','ocupada')
    AND h.id NOT IN (SELECT habitacion_id FROM reservas WHERE tenant_id=? AND estado NOT IN ('cancelada','checkout','noshow') AND fecha_entrada<? AND fecha_salida>?)
    ORDER BY h.piso,h.numero`).bind(tenant_id, tenant_id, fecha_salida, fecha_entrada).all();
...
reservasWeb.post('/reservar', async c => {
  ...
  if (existe) { huespedId = existe.id; }
    ...
    const { meta } = await c.env.DB.prepare('INSERT INTO huespedes (tenant_id,nombre,apellido,email,telefono,nacionalidad,documento_tipo,documento_numero) VALUES (?,?,?,?,?,?,?,?)').bind(tenant_id, huesped.nombre, huesped.apellido, huesped.email, huesped.telefono||null, huesped.nacionalidad||null, huesped.documento_tipo||null, huesped.documento_numero||null).run();
    huespedId = meta.last_row_id;
  }

  const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
  let codigo = 'WEB-' + Array.from({length:6}, () => chars[Math.floor(Math.random()*chars.length)]).join('');
  while (await c.env.DB.prepare('SELECT id FROM reservas WHERE codigo=? AND tenant_id=?').bind(codigo, tenant_id).first()) codigo = 'WEB-' + Array.from({length:6}, () => chars[Math.floor(Math.random()*chars.length)]).join('');

  const [insRes, habInfo] = await Promise.all([
    c.env.DB.prepare(`INSERT INTO reservas (tenant_id,codigo,huesped_id,habitacion_id,fecha_entrada,fecha_salida,adultos,ninos,estado,origen,precio_total,precio_noche,noches,senia) VALUES (?,?,?,?,?,?,?,?,'pendiente','web',?,?,?,?)`)
      .bind(tenant_id, codigo, huespedId, habitacion_id, fecha_entrada, fecha_salida, adultos||1, ninos||0, precio_total, precio_noche, noches, seniaVal).run(),
    c.env.DB.prepare('SELECT numero FROM habitaciones WHERE id=? AND tenant_id=?').bind(habitacion_id, tenant_id).first(),
  ]);
  const reservaId = insRes.meta.last_row_id;

  await c.env.DB.prepare('INSERT INTO notificaciones (tenant_id,tipo,titulo,mensaje,link,referencia_id,referencia_tipo) VALUES (?,?,?,?,?,?,?)').bind(tenant_id, 'reserva_web', `Nueva reserva web — Hab. ${habInfo?.numero||habitacion_id}`, `${huesped.nombre} ${huesped.apellido} · ${fecha_entrada} → ${fecha_salida} · ${noches} noche(s)`, '/reservas', reservaId, 'reservas').run();
...
reservasWeb.get('/ocupacion/:hab_id', async c => {
  ...
  const { results } = await c.env.DB.prepare(`SELECT fecha_entrada,fecha_salida FROM reservas WHERE habitacion_id=? AND tenant_id=? AND estado NOT IN ('cancelada','checkout','noshow') AND fecha_salida>? AND fecha_entrada<?`).bind(c.req.param('hab_id'), tenant_id, hoy, finStr).all();
