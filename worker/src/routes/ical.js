...
ical.get('/:hab_id.ics', async c => {
  ...
  const [hab, cfgRow] = await Promise.all([
    c.env.DB.prepare(`SELECT h.id,h.numero,t.nombre as tipo_nombre FROM habitaciones h JOIN tipos_habitacion t ON h.tipo_id=t.id WHERE h.id=?`).bind(hab_id).first(),
    c.env.DB.prepare(`SELECT valor FROM configuracion WHERE clave='nombre_hosteria'`).first(),
  ]);

  if (!hab) return c.text('Habitación no encontrada', 404);

  const nombreHosteria = cfgRow?.valor || 'Hostería';
  const { results: reservas } = await c.env.DB.prepare(`SELECT r.codigo,r.fecha_entrada,r.fecha_salida,r.adultos,r.ninos,h.nombre||' '||h.apellido as huesped_nombre FROM reservas r JOIN huespedes h ON r.huesped_id=h.id WHERE r.habitacion_id=? AND r.estado NOT IN ('cancelada','checkout','noshow') AND r.fecha_salida>=date('now') ORDER BY r.fecha_entrada`).bind(hab_id).all();
