...
consumos.get('/reserva/:reserva_id', authMiddleware, async c => {
  const { results } = await c.env.DB.prepare(`SELECT c.*,s.nombre as servicio_nombre,s.categoria FROM consumos c LEFT JOIN servicios s ON c.servicio_id=s.id WHERE c.reserva_id=? ORDER BY c.fecha DESC`).bind(c.req.param('reserva_id')).all();
...
consumos.post('/', authMiddleware, async c => {
  ...
  const { meta } = await c.env.DB.prepare('INSERT INTO consumos (reserva_id,servicio_id,descripcion,cantidad,precio_unitario,total) VALUES (?,?,?,?,?,?)')
    .bind(reserva_id, servicio_id||null, descripcion, cantNum, precioNum, total).run();
  return c.json({ id: meta.last_row_id, mensaje: 'Consumo registrado' }, 201);
});

consumos.delete('/:id', authMiddleware, async c => {
  await c.env.DB.prepare('DELETE FROM consumos WHERE id=?').bind(c.req.param('id')).run();
