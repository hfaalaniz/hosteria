...
partesLimpieza.get('/', authMiddleware, async c => {
  ...
  let q = `SELECT pl.*,hab.numero as habitacion_numero,hab.piso,u.nombre as usuario_nombre FROM partes_limpieza pl JOIN habitaciones hab ON pl.habitacion_id=hab.id JOIN usuarios u ON pl.usuario_id=u.id WHERE pl.tenant_id=?`;
...
partesLimpieza.get('/:id', authMiddleware, async c => {
  const { tenant_id } = c.get('user');
  const p = await c.env.DB.prepare(`SELECT pl.*,hab.numero as habitacion_numero,hab.piso,u.nombre as usuario_nombre FROM partes_limpieza pl JOIN habitaciones hab ON pl.habitacion_id=hab.id JOIN usuarios u ON pl.usuario_id=u.id WHERE pl.id=? AND pl.tenant_id=?`).bind(c.req.param('id'), tenant_id).first();
...
partesLimpieza.post('/', authMiddleware, async c => {
  ...
  const { meta } = await c.env.DB.prepare(`INSERT INTO partes_limpieza (tenant_id,habitacion_id,usuario_id,fecha,turno,estado_habitacion,tareas_realizadas,hallazgos,objetos_encontrados,insumos_usados,insumos_faltantes,ropa_cama,toallas,amenities,minibar,observaciones,tiempo_minutos) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
    .bind(tenant_id, habitacion_id, user.id, fecha || new Date().toISOString().split('T')[0], turno || 'manana', estado_habitacion || 'limpia', JSON.stringify(tareas_realizadas || []), hallazgos || null, objetos_encontrados || null, JSON.stringify(insumos_usados || []), JSON.stringify(insumos_faltantes || []), ropa_cama || 'ok', toallas || 'ok', amenities || 'ok', minibar || 'no_aplica', observaciones || null, tiempo_minutos || null).run();
  ...
  if ((estado_habitacion || 'limpia') === 'limpia') await c.env.DB.prepare(`UPDATE habitaciones SET estado='disponible' WHERE id=? AND tenant_id=? AND estado='limpieza'`).bind(habitacion_id, tenant_id).run();

  if (objetos_encontrados) notifStmts.push(c.env.DB.prepare('INSERT INTO notificaciones (tenant_id,tipo,titulo,mensaje,link,referencia_id,referencia_tipo) VALUES (?,?,?,?,?,?,?)').bind(tenant_id, 'otro', `Objetos encontrados — Hab. ${hab?.numero || habitacion_id}`, objetos_encontrados, '/partes-limpieza', parteId, 'partes_limpieza'));
  if (estado_habitacion === 'danio') notifStmts.push(c.env.DB.prepare('INSERT INTO notificaciones (tenant_id,tipo,titulo,mensaje,link,referencia_id,referencia_tipo) VALUES (?,?,?,?,?,?,?)').bind(tenant_id, 'mantenimiento_urgente', `Daño reportado en limpieza — Hab. ${hab?.numero || habitacion_id}`, hallazgos || 'Se reportó un daño durante el parte de limpieza.', '/partes-limpieza', parteId, 'partes_limpieza'));
...
partesLimpieza.put('/:id', authMiddleware, async c => {
  ...
  await c.env.DB.prepare(`UPDATE partes_limpieza SET turno=?,estado_habitacion=?,tareas_realizadas=?,hallazgos=?,objetos_encontrados=?,insumos_usados=?,insumos_faltantes=?,ropa_cama=?,toallas=?,amenities=?,minibar=?,observaciones=?,tiempo_minutos=? WHERE id=? AND tenant_id=?`)
    .bind(turno, estado_habitacion, JSON.stringify(tareas_realizadas || []), hallazgos, objetos_encontrados, JSON.stringify(insumos_usados || []), JSON.stringify(insumos_faltantes || []), ropa_cama, toallas, amenities, minibar, observaciones, tiempo_minutos, c.req.param('id'), tenant_id).run();
  return c.json({ mensaje: 'Parte actualizado' });
});

partesLimpieza.delete('/:id', authMiddleware, async c => {
  const { tenant_id } = c.get('user');
  await c.env.DB.prepare('DELETE FROM partes_limpieza WHERE id=? AND tenant_id=?').bind(c.req.param('id'), tenant_id).run();
