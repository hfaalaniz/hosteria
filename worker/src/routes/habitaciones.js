...
habitaciones.get('/', authMiddleware, async c => {
  ...
  let query = `SELECT h.*, t.nombre as tipo_nombre, t.precio_base, t.precio_fin_semana, t.capacidad, t.amenidades,
    CASE WHEN h.estado IN ('mantenimiento','limpieza') THEN h.estado
         ...
         WHEN EXISTS (SELECT 1 FROM reservas r WHERE r.habitacion_id=h.id AND r.tenant_id=h.tenant_id AND r.estado NOT IN ('cancelada','checkout','noshow') AND r.fecha_entrada<=? AND r.fecha_salida>?) THEN 'ocupada'
         ELSE 'disponible' END as estado_real
    FROM habitaciones h JOIN tipos_habitacion t ON h.tipo_id=t.id WHERE h.tenant_id=?`;
  const params = [hoy, hoy, tenant_id];
  if (estado === 'disponible') {
    query += ` AND h.estado NOT IN ('mantenimiento','limpieza','ocupada') AND NOT EXISTS (SELECT 1 FROM reservas r WHERE r.habitacion_id=h.id AND r.tenant_id=h.tenant_id AND r.estado NOT IN ('cancelada','checkout','noshow') AND r.fecha_entrada<=? AND r.fecha_salida>?)`;
    params.push(hoy, hoy);
...
habitaciones.get('/disponibles', authMiddleware, async c => {
  ...
  const { results } = await c.env.DB.prepare(`
    ...
    WHERE h.tenant_id=? AND h.estado NOT IN ('mantenimiento','limpieza')
      AND (? IS NULL OR t.capacidad >= ?)
      AND NOT EXISTS (SELECT 1 FROM reservas r WHERE r.habitacion_id=h.id AND r.tenant_id=h.tenant_id AND r.estado NOT IN ('cancelada','checkout','noshow') AND r.fecha_entrada<? AND r.fecha_salida>?)
    ORDER BY h.piso, h.numero`).bind(tenant_id, adultos || null, adultos || null, fecha_salida, fecha_entrada).all();
...
habitaciones.get('/disponibilidad/check', authMiddleware, async c => {
  ...
  let q = `SELECT id FROM reservas WHERE habitacion_id=? AND tenant_id=? AND estado NOT IN ('cancelada','checkout','noshow') AND fecha_entrada<? AND fecha_salida>?`;
  const params = [habitacion_id, tenant_id, fecha_salida, fecha_entrada];
...
habitaciones.get('/:id', authMiddleware, async c => {
  const { tenant_id } = c.get('user');
  const h = await c.env.DB.prepare(`SELECT h.*, t.nombre as tipo_nombre, t.precio_base, t.precio_fin_semana, t.capacidad, t.amenidades FROM habitaciones h JOIN tipos_habitacion t ON h.tipo_id=t.id WHERE h.id=? AND h.tenant_id=?`).bind(c.req.param('id'), tenant_id).first();
...
habitaciones.post('/', authMiddleware, async c => {
  ...
  try {
    const { meta } = await c.env.DB.prepare('INSERT INTO habitaciones (tenant_id,numero,piso,tipo_id,notas,camas,precio_override,descripcion,metros_cuadrados,vista,bano_privado) VALUES (?,?,?,?,?,?,?,?,?,?,?)')
      .bind(tenant_id, numero, piso||1, tipo_id, notas||null, JSON.stringify(camas||[]), precio_override||null, descripcion||null, metros_cuadrados||null, vista||null, bano_privado??1).run();
...
habitaciones.put('/:id', authMiddleware, async c => {
  ...
  const h = await c.env.DB.prepare('SELECT id FROM habitaciones WHERE id=? AND tenant_id=?').bind(c.req.param('id'), tenant_id).first();
  if (!h) return c.json({ error: 'Habitación no encontrada' }, 404);
  await c.env.DB.prepare('UPDATE habitaciones SET numero=?,piso=?,tipo_id=?,estado=?,notas=?,camas=?,precio_override=?,descripcion=?,metros_cuadrados=?,vista=?,bano_privado=? WHERE id=? AND tenant_id=?')
    .bind(numero,piso,tipo_id,estado,notas,JSON.stringify(camas||[]),precio_override||null,descripcion||null,metros_cuadrados||null,vista||null,bano_privado??1,c.req.param('id'),tenant_id).run();
...
habitaciones.patch('/:id/estado', authMiddleware, async c => {
  ...
  await c.env.DB.prepare('UPDATE habitaciones SET estado=? WHERE id=? AND tenant_id=?').bind(estado, c.req.param('id'), tenant_id).run();
  return c.json({ mensaje: 'Estado actualizado' });
});

habitaciones.delete('/:id', authMiddleware, async c => {
  const { tenant_id } = c.get('user');
  const ocupada = await c.env.DB.prepare("SELECT id FROM reservas WHERE habitacion_id=? AND tenant_id=? AND estado IN ('checkin','confirmada')").bind(c.req.param('id'), tenant_id).first();
  if (ocupada) return c.json({ error: 'No se puede eliminar una habitación con reservas activas' }, 400);
  await c.env.DB.prepare('DELETE FROM habitaciones WHERE id=? AND tenant_id=?').bind(c.req.param('id'), tenant_id).run();
