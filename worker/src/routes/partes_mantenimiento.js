...
partesMantenimiento.get('/', authMiddleware, async c => {
  ...
  let q = `SELECT pm.*,hab.numero as habitacion_numero,hab.piso,u.nombre as usuario_nombre FROM partes_mantenimiento pm JOIN habitaciones hab ON pm.habitacion_id=hab.id JOIN usuarios u ON pm.usuario_id=u.id WHERE 1=1`;
  const params = [];
...
partesMantenimiento.get('/:id', authMiddleware, async c => {
  const p = await c.env.DB.prepare(`SELECT pm.*,hab.numero as habitacion_numero,hab.piso,u.nombre as usuario_nombre FROM partes_mantenimiento pm JOIN habitaciones hab ON pm.habitacion_id=hab.id JOIN usuarios u ON pm.usuario_id=u.id WHERE pm.id=?`).bind(c.req.param('id')).first();
...
partesMantenimiento.post('/', authMiddleware, async c => {
  ...
  if (['alta', 'urgente'].includes(prioridad) || tipo === 'urgente') {
    ...
    await c.env.DB.batch([
      c.env.DB.prepare("UPDATE habitaciones SET estado='mantenimiento' WHERE id=?").bind(habitacion_id),
      c.env.DB.prepare('INSERT INTO notificaciones (tipo,titulo,mensaje,link,referencia_id,referencia_tipo) VALUES (?,?,?,?,?,?)').bind('mantenimiento_urgente', `Parte mantenimiento urgente — Hab. ${hab?.numero || habitacion_id}`, descripcion, '/partes-mantenimiento', parteId, 'partes_mantenimiento'),
    ]);
  }
...
partesMantenimiento.put('/:id', authMiddleware, async c => {
  ...
  const existente = await c.env.DB.prepare('SELECT resuelto_en,habitacion_id FROM partes_mantenimiento WHERE id=?').bind(id).first();
  const resuelto_en = estado === 'resuelto' ? (existente?.resuelto_en || new Date().toISOString()) : null;

  await c.env.DB.prepare(`UPDATE partes_mantenimiento SET tipo=?,prioridad=?,descripcion=?,diagnostico=?,trabajo_realizado=?,estado=?,materiales_usados=?,materiales_necesarios=?,requiere_externo=?,proveedor_externo=?,costo_estimado=?,costo_real=?,tiempo_minutos=?,observaciones=?,proxima_revision=?,resuelto_en=? WHERE id=?`)
    .bind(tipo, prioridad, descripcion, diagnostico, trabajo_realizado, estado, JSON.stringify(materiales_usados || []), JSON.stringify(materiales_necesarios || []), requiere_externo ? 1 : 0, proveedor_externo, costo_estimado, costo_real, tiempo_minutos, observaciones, proxima_revision, resuelto_en, id).run();

  if (existente?.habitacion_id) {
    if (estado === 'resuelto') {
      const otroPendiente = await c.env.DB.prepare(`SELECT id FROM partes_mantenimiento WHERE habitacion_id=? AND estado NOT IN ('resuelto','derivado') AND id!=?`).bind(existente.habitacion_id, id).first();
      if (!otroPendiente) await c.env.DB.prepare(`UPDATE habitaciones SET estado='disponible' WHERE id=? AND estado='mantenimiento'`).bind(existente.habitacion_id).run();
    } else if (estado === 'en_proceso' && prioridad === 'alta') {
      await c.env.DB.prepare(`UPDATE habitaciones SET estado='mantenimiento' WHERE id=?`).bind(existente.habitacion_id).run();
    }
  }
...
partesMantenimiento.delete('/:id', authMiddleware, async c => {
  await c.env.DB.prepare('DELETE FROM partes_mantenimiento WHERE id=?').bind(c.req.param('id')).run();
