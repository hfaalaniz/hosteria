...
reservas.get('/', authMiddleware, async c => {
  ...
  let q = `SELECT r.*, h.nombre||' '||h.apellido as huesped_nombre, h.email as huesped_email, h.telefono as huesped_telefono,
    ...
    FROM reservas r JOIN huespedes h ON r.huesped_id=h.id JOIN habitaciones hab ON r.habitacion_id=hab.id JOIN tipos_habitacion t ON hab.tipo_id=t.id WHERE 1=1`;
  const params = [];
...
reservas.get('/calendario', authMiddleware, async c => {
  ...
  const [{ results: habs }, { results: resv }] = await Promise.all([
    c.env.DB.prepare(`SELECT h.id,h.numero,h.piso,h.estado,t.nombre as tipo_nombre,t.capacidad FROM habitaciones h JOIN tipos_habitacion t ON h.tipo_id=t.id WHERE h.estado!='mantenimiento' ORDER BY h.piso,h.numero`).all(),
    c.env.DB.prepare(`SELECT r.id,r.codigo,r.habitacion_id,r.fecha_entrada,r.fecha_salida,r.estado,r.adultos,r.ninos,r.precio_total,r.origen,r.noches,hu.nombre||' '||hu.apellido as huesped_nombre FROM reservas r JOIN huespedes hu ON r.huesped_id=hu.id WHERE r.estado NOT IN ('cancelada','noshow') AND r.fecha_entrada<? AND r.fecha_salida>? ORDER BY r.fecha_entrada`).bind(fecha_fin, fecha_inicio).all(),
  ]);
...
reservas.post('/', authMiddleware, async c => {
  try {
    ...
    if (!huesped_id && huesped) {
      ...
      let existente = null;
      if (email) existente = await c.env.DB.prepare('SELECT id FROM huespedes WHERE email=?').bind(email).first();
      if (!existente && documento_numero) existente = await c.env.DB.prepare('SELECT id FROM huespedes WHERE documento_numero=?').bind(documento_numero).first();
      if (existente) { huesped_id = existente.id; }
      else {
        const { meta } = await c.env.DB.prepare('INSERT INTO huespedes (nombre,apellido,email,telefono,nacionalidad,documento_tipo,documento_numero) VALUES (?,?,?,?,?,?,?)')
          .bind(nombre, apellido, email||null, telefono||null, nacionalidad||null, documento_tipo||'DNI', documento_numero||null).run();
    ...
    while (await c.env.DB.prepare('SELECT id FROM reservas WHERE codigo=?').bind(codigo).first()) codigo = generarCodigo();

    // Check disponibilidad + INSERT atómico via batch
    ...
    const stmts = [
      c.env.DB.prepare('INSERT INTO reservas (codigo,huesped_id,habitacion_id,fecha_entrada,fecha_salida,adultos,ninos,estado,origen,precio_total,precio_noche,noches,notas) VALUES (?,?,?,?,?,?,?,\'confirmada\',?,?,?,?,?)')
        .bind(codigo, huesped_id, habitacion_id, fecha_entrada, fecha_salida, adultos||1, ninos||0, origen||'directo', precio_total, precioNocheNum, noches, notas||null),
    ];
    await c.env.DB.batch(stmts);
    const reserva = await c.env.DB.prepare('SELECT id FROM reservas WHERE codigo=?').bind(codigo).first();
    const reservaId = reserva.id;

    if (Array.isArray(acompanantes) && acompanantes.length > 0) {
      await c.env.DB.batch(acompanantes.filter(a => a.nombre).map(a =>
        c.env.DB.prepare('INSERT INTO reserva_acompanantes (reserva_id,tipo,nombre,documento_tipo,documento_numero,edad) VALUES (?,?,?,?,?,?)')
          .bind(reservaId, a.tipo||'adulto', a.nombre, a.documento_tipo||null, a.documento_numero||null, a.edad||null)
      ));
    }
...
reservas.put('/:id', authMiddleware, async c => {
  ...
  const r = await c.env.DB.prepare('SELECT * FROM reservas WHERE id=?').bind(c.req.param('id')).first();
  if (!r) return c.json({ error: 'Reserva no encontrada' }, 404);
  const noches = calcularNoches(fecha_entrada, fecha_salida);
  await c.env.DB.prepare('UPDATE reservas SET fecha_entrada=?,fecha_salida=?,adultos=?,ninos=?,origen=?,precio_noche=?,precio_total=?,noches=?,notas=?,notas_internas=?,actualizado_en=CURRENT_TIMESTAMP WHERE id=?')
    .bind(fecha_entrada, fecha_salida, adultos, ninos, origen, precio_noche, precio_noche*noches, noches, notas, notas_internas, c.req.param('id')).run();
...
reservas.patch('/:id/notas', authMiddleware, async c => {
  ...
  await c.env.DB.prepare('UPDATE reservas SET notas=?,notas_internas=?,actualizado_en=CURRENT_TIMESTAMP WHERE id=?').bind(notas??null, notas_internas??null, c.req.param('id')).run();
...
reservas.patch('/:id/senia', authMiddleware, async c => {
  ...
  await c.env.DB.prepare('UPDATE reservas SET senia_cobrada=1,senia_metodo=?,senia_fecha=?,actualizado_en=CURRENT_TIMESTAMP WHERE id=?').bind(metodo, fecha, c.req.param('id')).run();
  if (reserva.huesped_email) emailSeniaCobrada(c.env.DB, { ...reserva, senia_metodo: metodo }).catch(() => {});
...
reservas.post('/:id/recordatorio', authMiddleware, async c => {
  ...
  try {
    await emailRecordatorio(c.env.DB, reserva);
...
reservas.patch('/:id/estado', authMiddleware, async c => {
  ...
  const reserva = await c.env.DB.prepare('SELECT * FROM reservas WHERE id=?').bind(c.req.param('id')).first();
  if (!reserva) return c.json({ error: 'Reserva no encontrada' }, 404);
  ...
  if (estado === 'cancelada') {
    ...
    stmts.push(c.env.DB.prepare('UPDATE reservas SET estado=?,cancelacion_penalidad=?,cancelacion_dias_anticipacion=?,actualizado_en=CURRENT_TIMESTAMP WHERE id=?').bind(estado, penalidad, diasAnticipacion, c.req.param('id')));
  } else {
    stmts.push(c.env.DB.prepare('UPDATE reservas SET estado=?,actualizado_en=CURRENT_TIMESTAMP WHERE id=?').bind(estado, c.req.param('id')));
  }

  if (estado === 'checkin') stmts.push(c.env.DB.prepare("UPDATE habitaciones SET estado='ocupada' WHERE id=?").bind(reserva.habitacion_id));
  else if (estado === 'checkout') stmts.push(c.env.DB.prepare("UPDATE habitaciones SET estado='limpieza' WHERE id=?").bind(reserva.habitacion_id));
  else if (estado === 'cancelada' || estado === 'noshow') {
    const hab = await c.env.DB.prepare("SELECT estado FROM habitaciones WHERE id=?").bind(reserva.habitacion_id).first();
    if (hab?.estado === 'ocupada') stmts.push(c.env.DB.prepare("UPDATE habitaciones SET estado='disponible' WHERE id=?").bind(reserva.habitacion_id));
  }
  await c.env.DB.batch(stmts);
...
reservas.patch('/:id/extender', authMiddleware, async c => {
  ...
  const conflicto = await c.env.DB.prepare(`SELECT id FROM reservas WHERE habitacion_id=? AND id!=? AND estado NOT IN ('cancelada','checkout','noshow') AND fecha_entrada<? AND fecha_salida>?`).bind(reserva.habitacion_id, reserva.id, nueva_fecha_salida, reserva.fecha_salida).first();
  ...
  await c.env.DB.prepare('UPDATE reservas SET fecha_salida=?,noches=?,precio_total=?,actualizado_en=CURRENT_TIMESTAMP WHERE id=?').bind(nueva_fecha_salida, nuevasNoches, nuevoPrecioTotal, c.req.param('id')).run();
