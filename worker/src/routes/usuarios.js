...
usuarios.get('/', authMiddleware, adminOnly, async c => {
  const { results } = await c.env.DB.prepare('SELECT id, nombre, email, rol, activo, creado_en FROM usuarios ORDER BY nombre').all();
...
usuarios.post('/', authMiddleware, adminOnly, async c => {
  ...
  const { meta } = await c.env.DB.prepare('INSERT INTO usuarios (nombre, email, password, rol) VALUES (?, ?, ?, ?)').bind(nombre, email, hashed, rol).run();
...
usuarios.put('/:id', authMiddleware, adminOnly, async c => {
  ...
  const existe = await c.env.DB.prepare('SELECT id FROM usuarios WHERE email = ? AND id != ?').bind(email, c.req.param('id')).first();
  if (existe) return c.json({ error: 'Ese email ya está en uso' }, 409);

  await c.env.DB.prepare('UPDATE usuarios SET nombre=?, email=?, rol=?, activo=? WHERE id=?').bind(nombre, email, rol, activo ?? 1, c.req.param('id')).run();
...
usuarios.patch('/:id/password', authMiddleware, async c => {
  ...
  const hashed = await hash(password, 10);
  await c.env.DB.prepare('UPDATE usuarios SET password=? WHERE id=?').bind(hashed, c.req.param('id')).run();
...
usuarios.patch('/:id/activo', authMiddleware, adminOnly, async c => {
  ...
  await c.env.DB.prepare('UPDATE usuarios SET activo=? WHERE id=?').bind(activo ? 1 : 0, c.req.param('id')).run();
