...
huespedes.get('/', authMiddleware, async c => {
  ...
  const { results } = await c.env.DB.prepare(`
    ...
    FROM huespedes h LEFT JOIN reservas r ON r.huesped_id=h.id AND r.tenant_id=h.tenant_id
...
huespedes.get('/:id', authMiddleware, async c => {
  const { tenant_id } = c.get('user');
  const h = await c.env.DB.prepare('SELECT * FROM huespedes WHERE id=? AND tenant_id=?').bind(c.req.param('id'), tenant_id).first();
  ...
  const { results: reservas } = await c.env.DB.prepare(`
    ...
    WHERE r.huesped_id=? AND r.tenant_id=? ORDER BY r.fecha_entrada DESC`).bind(c.req.param('id'), tenant_id).all();
...
huespedes.post('/', authMiddleware, async c => {
  ...
  const { meta } = await c.env.DB.prepare('INSERT INTO huespedes (tenant_id,nombre,apellido,email,telefono,nacionalidad,documento_tipo,documento_numero,direccion,notas) VALUES (?,?,?,?,?,?,?,?,?,?)')
    .bind(tenant_id, nombre, apellido, email||null, telefono||null, nacionalidad||null, documento_tipo||'DNI', documento_numero||null, direccion||null, notas||null).run();
...
huespedes.put('/:id', authMiddleware, async c => {
  const { tenant_id } = c.get('user');
  const { nombre, apellido, email, telefono, nacionalidad, documento_tipo, documento_numero, direccion, notas } = await c.req.json();
  await c.env.DB.prepare('UPDATE huespedes SET nombre=?,apellido=?,email=?,telefono=?,nacionalidad=?,documento_tipo=?,documento_numero=?,direccion=?,notas=? WHERE id=? AND tenant_id=?')
    .bind(nombre, apellido, email||null, telefono||null, nacionalidad||null, documento_tipo||'DNI', documento_numero||null, direccion||null, notas||null, c.req.param('id'), tenant_id).run();
  return c.json({ mensaje: 'Huésped actualizado' });
});

huespedes.delete('/:id', authMiddleware, async c => {
  const { tenant_id } = c.get('user');
  const activo = await c.env.DB.prepare("SELECT id FROM reservas WHERE huesped_id=? AND tenant_id=? AND estado IN ('checkin','confirmada','pendiente')").bind(c.req.param('id'), tenant_id).first();
  if (activo) return c.json({ error: 'No se puede eliminar un huésped con reservas activas' }, 400);
  await c.env.DB.prepare('DELETE FROM huespedes WHERE id=? AND tenant_id=?').bind(c.req.param('id'), tenant_id).run();
...
huespedes.post('/:id/foto-documento', authMiddleware, async c => {
  ...
  const key = `docs/doc_${tenant_id}_${c.req.param('id')}_${Date.now()}.${ext}`;
  await c.env.R2.put(key, await file.arrayBuffer(), { httpMetadata: { contentType: file.type } });
  const url = `/imgs/${key}`;
  await c.env.DB.prepare('UPDATE huespedes SET foto_documento=? WHERE id=? AND tenant_id=?').bind(url, c.req.param('id'), tenant_id).run();
  return c.json({ url });
});
